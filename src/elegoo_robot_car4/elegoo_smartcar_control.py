...
class GameEngine:
    ...
    __key_to_cmd: dict[int, Callable[[Car], Any]] = {
    ...
    def run(self) -> None:
        ...
        while True:
            ...
            for move_key in self.__key_to_cmd:
                if was_pressed[move_key]:
                    self.__key_to_cmd[move_key](self.__car)
                    move_command_received = True
            if not move_command_received:
                for stick in joysticks.values():
                    move_command_received = self.__controller_handler(
                        stick, joybuttons
                    )
            if not move_command_received:
    ...
    def __controller_handler(
        self, stick: pg.joystick.Joystick, buttons: list[int]
    ) -> bool:
        ...
        cur_speed = round(
            np.clip(self.__min_speed + speed_axis * self.__delta_speed, 0, 255)
        )
