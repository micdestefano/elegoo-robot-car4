...
- Now `ruff` and `prek` are run through `uvx` so they're not dependencies
  anymore.
...
- Project now uses `just` and not `make` for configuration.
- Project now uses `prek` and not `pre-commit` for running hooks.
